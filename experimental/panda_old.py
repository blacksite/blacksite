...
import logging
import time
from immunity.models import NeuralNetworkNSA
import numpy as np
...
def evaulate_model(w, grizzly, index):
    global DATASET
    ...
    results = model.predict(np.array(test_x), np.array(test_y), DATASET.CLASSES)
    ...
    for c in DATASET.CLASSES:
        w.write(',{:^30.0f},{:^30.0f}'.format(results[c][0], results[c][1]))
    w.write("\n")
    w.flush()
