...
from cache.detector import Detector
...
class NeuralNetworkNSA:
    ...
    def generate_detectors(self):
        ...
        start = time.time()
        # threads = list()
        self.generate_detectors_thread_by_sample_type()
        # for i in range(10):
            # random_type = random.randrange(len(seed))
            # self.futures.append(self.executor.submit(self.generate_detectors_thread_by_sample_type))
            # x = threading.Thread(target=self.generate_detectors_thread, args=(grizzly, i, seed, classes, ))
            # threads.append(x)
            # x.start()
        # self.futures.append(self.executor.submit(self.transfer_detector_values))
    ...
    def generate_detectors_thread(self):
        ...
        while True:
            detector = Detector(num_features=self.number_of_features,
                                seed=self.data_set.get_min_max_features(self.device))

            classification = self.validator.classify(detector.get_mean_value())
    ...
    def generate_detectors_thread_by_sample_type(self):
    ...
    def validate_r_values(self, maxes, number_of_samples, labels):
        ...
        results = {}
        for c in range(1, self.data_set.get_number_of_classes()):
            results[c] = {}
            for r_value in range(self.number_of_features):
                results[c][r_value] = [0, 0, 0, 0]

        for r_value in range(1, self.number_of_features):

            for sample_index in range(int(math.sqrt(number_of_threads))):
                # self.validate_r_value_kernel(maxes, results, r_value, sample_index, number_of_samples, labels)
                self.futures.append(self.executor.submit(self.validate_r_value_kernel, maxes, results, r_value, sample_index, number_of_samples, labels))

        wait(self.futures, return_when='ALL_COMPLETED')

        for c in range(1, self.data_set.get_number_of_classes()):
            self.best_tp[c] = -1.0
            self.best_accuracy[c] = -1.0
            for r_value in range(1, self.number_of_features):
                # print(str(r_value))

                tp = results[c][r_value][0]
                fn = results[c][r_value][1]
                tn = results[c][r_value][2]
                fp = results[c][r_value][3]

                if tp+fn != 0 and tn+fp != 0:
                    if tn+fp > tp+fn:
                        percentage = (tp+fn)/(tn+fp)
                        tn = int(percentage * tn)
                        fp = int(percentage * fp)

                    accuracy = (tp+tn)/(tp+fn+tn+fp)
                    if accuracy > self.best_accuracy[c]:
                        self.best_accuracy[c] = accuracy
                        self.best_r_value[c] = r_value
            ...
            if sample_index < number_of_samples:
                ...
                if classification != 0:
                    ...
                    if maxes[sample_index] > r_value:
                        results[classification][r_value][0] += 1
                    # False Negative
                    else:
                        results[classification][r_value][1] += 1

                else:
                    for c in range(1, self.data_set.get_number_of_classes()):
                        if maxes[sample_index] > r_value:
                            # False Positive
                            results[c][r_value][3] += 1
                        else:
                            # True Negative
                            results[c][r_value][2] += 1
                self.lock.release()
    ...
    def test_r_value(self, maxes, number_of_samples, labels):
        ...
        for sample_index in range(int(math.sqrt(number_of_threads))):
            # self.test_r_value_kernel(maxes, sample_index, number_of_samples, labels)
            self.futures.append(self.executor.submit(self.test_r_value_kernel, maxes, sample_index, number_of_samples, labels))

        wait(self.futures, return_when='ALL_COMPLETED')

    def test_r_value_kernel(self, maxes, sample_index, number_of_samples, labels):
        global number_of_threads
        ...
        for m in range(int(number_of_samples)):
            ...
            if sample_index < number_of_samples:
                ...
                if encoded_label == 0:
                    detected = False
                    for mal_type, r_value in self.best_r_value.items():
                        if maxes[sample_index] > r_value:
                    ...
                    if maxes[sample_index] > self.best_r_value[encoded_label]:
                        self.best_results[original_class][0] += 1
                    else:
                        self.best_results[original_class][1] += 1
                self.lock.release()
        ...
        for m in range(number_of_samples):
            ...
            if sample_index < number_of_samples:
                ...
                for i in range(number_of_features):
                    ...
                    if min_val <= samp and max_val >= samp:
                        count += 1
